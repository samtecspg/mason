...
class TestCLI:
    ...
    def test_play(self):
        ...
        print_result(runner.invoke(config, ["-s", "6"]))
        # print_result(runner.invoke(config, ["-s", "2"]))
        print_result(runner.invoke(run, ["operator", "table", "get", "-p", "database_name:mason-test-data,table_name:csv/test.csv", "-l", "trace"], catch_exceptions=False))
        # print_result(runner.invoke(run, ["operator", "table", "summarize", "-p", "database_name:mason-test-data,table_name:csv/test.csv,read_headers:true", "-l", "trace"], catch_exceptions=False))