...
class AthenaMock:
    ...
    def start_query_execution(*args, **kwargs):
        ...
        ddl = """CREATE EXTERNAL TABLE IF NOT EXISTS `test-database`.`test-table` (\n`col_0` STRING,\n`col_1` STRING\n)\nSTORED AS PARQUET\nLOCATION \'s3://test-database/test-table\'\n"""
        ...
        elif (kwargs["QueryExecutionContext"]["Database"] == "good_database") and (kwargs["QueryString"] == "SELECT * from good_table limit 3") and kwargs["WorkGroup"] == "mason":
        ...
        elif (kwargs["QueryExecutionContext"]["Database"] == "test-database" and clean_string(kwargs["QueryString"]) == clean_string(ddl)):
