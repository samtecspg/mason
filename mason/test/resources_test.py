...
class TestGetAll:
    ...
    def test_get_resources(self):
        env = base.get_env("/test/support/", "/test/support/validations/")
        resources = Resources(env)
        operators, bad = resources.get_operators()
        assert(len(operators) == 6)
        workflows, bad = resources.get_workflows() 
        assert(len(workflows) == 10)
        configs, bad = resources.get_configs()
        assert(len(configs) == 4)
        bad = resources.get_bad()
        assert(len(bad) == 8)

        namespace1, bad = resources.get_operators("namespace1")
        assert(len(namespace1) == 2)
        testing_namespace, bad = resources.get_workflows("testing_namespace")
        assert(len(testing_namespace) == 10)
        config5, bad = resources.get_configs("5")
