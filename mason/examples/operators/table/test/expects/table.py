...
def index(client_type: str, exists=True):
    ...
    if client_type == "glue":
        ...
        if exists:
            return ({'Data': [{'Prefix': '/test_path/'}], 'Errors': ["No valid tables at crawler-poc.  Try appending '/' or specify deeper key."]},
             200)
        else:
...
def get(client_type: str, switch: int):
    if client_type == "glue":
        if switch == 1:
            ...
            return ({'Errors': ['Database bad-database or table catalog_poc_data not found']}, 404)
        elif switch == 3:
            return ({'Errors': ['Database crawler-poc or table bad-table not found']}, 404)
        else:
            return {}
    elif client_type == "s3":
        if switch == 1:
