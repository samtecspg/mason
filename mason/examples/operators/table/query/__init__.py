...
class TableQuery(OperatorDefinition):
    def run(self, env: MasonEnvironment, config: ValidConfig, parameters: ValidatedParameters, response: Response) -> OperatorResponse:
        ...
        table, response = config.metastore.client.get_table(database_name, table_name)
        
        if output_path and not isinstance(config.storage.client, EmptyClient):
            outp: Optional[Path] = config.storage.client.path(output_path)
        else:
            outp = None
