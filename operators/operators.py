...
def from_config(config: dict):
    ...
    required_parameters = config.get("parameters", {}).get("required", [])
    supported_clients = config.get("supported_clients", [])
    if namespace and command:
        return Operator(namespace, command, required_parameters, supported_clients)
    else:
        None
...
def get_operator(config: Config, cmd: Optional[str], subcmd: Optional[str]) -> Optional[Operator]:
    ops: List[Operator] = list_operators(config, cmd)[cmd]
    op: Operator = list(filter(lambda x: x.subcommand == subcmd, ops))[0]
    return op
