...
then
  commands=(
      ...
      "mason operator table infer -p database_name:test-database,storage_path:spg-mason-demo/part_data_parts -l trace >> infer.txt"
      "cat infer.txt"
      "export JOB_ID=\"\$(cat infer.txt | grep \"job id\" | tail -n 1 | cut -d= -f2 | sed -e 's/\"$//')\""
      "echo \$JOB_ID"
      "mason config -s 1"
      "mason operator job get -p \"job_id:\$JOB_ID\" -l trace"
      "mason operator table query -p \"database_name:test-database,query_string:SELECT * from \\\"part_data_parts\\\" limit 5\" >> query.txt"
      "cat query.txt"
      "export JOB_ID=\"\$(cat query.txt | grep \"job id\" | tail -n 1 | cut -d= -f2 | sed -e 's/\"$//')\""
      "echo \$JOB_ID"
      "mason operator job get -p \"job_id:\$JOB_ID\""
