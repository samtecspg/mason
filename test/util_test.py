...
class TestJsonSchema:
    ...
    def test_basic_json_schema(self):
        test = from_json_schema({"test": "test"}, BASIC_SCHEMA, TestBasic)
        if test:
            ...
            try:
                ...
                assert(str(e) == '\'Test\' object has no attribute \'other\'')

    def test_basic_json_schema_invalid(self):
        logger.set_level("fatal")
        test = from_json_schema({"test": "test"}, BASIC_SCHEMA, TestBad)
        assert(test == None)
